...
var (
	inBytesCounter = prometheus.NewGaugeVec(
		...
		[]string{"ip", "hostname", "mac_address"})
...
var recordMetrics = func() {
	...
	for _, ip := range ipList {
		...
		inBytesCounter.WithLabelValues(ip, d.Hostname, d.MacAddress).Set(float64(d.In))
		outBytesCounter.WithLabelValues(ip, d.Hostname, d.MacAddress).Set(float64(d.Out))
		totalBytesCounter.WithLabelValues(ip, d.Hostname, d.MacAddress).Set(float64(d.Total))
	}
}
